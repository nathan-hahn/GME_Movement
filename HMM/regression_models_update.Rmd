...
```{r}
mod.df <- read.csv("individual_year_classification.csv")
...
```{r}
m1 <- lmer(year.mean ~ subject_sex + subject_ageClass + year.mcp.area + (1|subject_name), data = mod.df, REML = FALSE)
#summary(m1)

m2 <- lmer(year.mean ~ subject_sex + subject_ageClass + (1|subject_name), data = mod.df, REML = FALSE)
#summary(m2)

m3 <- lmer(year.mean ~ subject_sex + year.mcp.area + (1|subject_name), data = mod.df, REML = FALSE)
summary(m3)
...
```{r}
dat <- ggpredict(m3, terms = c("year.mcp.area", "subject_sex"))
plot(dat)
