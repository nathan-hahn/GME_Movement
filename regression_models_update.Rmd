...
m7 <- lmer(year.mean ~ log(year.mcp.area) + (1|subject_name), data = mod.df, REML = FALSE)

m8 <- lmer(year.mean ~ log(mu.daily.disp) + (1|subject_name), data = mod.df, REML = FALSE)

m9 <- lmer(year.mean ~ log(mu.daily.disp) + log(year.mcp.area) + (1|subject_name), data = mod.df, REML = FALSE)

knitr::kable(AICc(m1, m2, m3, m4, m5, m6, m7, m8, m9))

sjPlot::tab_model(m9)

# mod selection table
aicc <- AICc(m1, m2, m3, m4, m5, m6, m7, m8, m9)
