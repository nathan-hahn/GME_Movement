...
```{r}
mod <- lmer(year.mean ~ subject_sex + log(year.mcp.area) + log(mu.daily.disp) + (1|subject_name), data = mod.df, REML = FALSE)
#mod <- lmer(year.mean ~ subject_sex + log(year.mcp.area) + log(mu.daily.disp) + log(centroid.dist+0.01) + (1|subject_name), data = mod.df, REML = FALSE)
summary(mod)
confint(mod)
...
plot(global, year.mean ~ fitted(.), abline = c(0,1))
```

Use the fitted model, see that males use more ag than females, but not significantly more.
```{r}
dat <- ggpredict(global, terms = c("mu.daily.disp", "ag.prop.hr"))
plot(dat)
