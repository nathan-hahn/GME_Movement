...
output <- readRDS("./HMM/EleCollars_211119_HMMclassified_2019-11-30.rds")
dist <- readRDS("EleCollars_211119_usedClust_2019-11-28.rds")
...
budget <- output %>%
  ...
  mutate(viterbi = recode_factor(viterbi,
                                 "1" = "encamped",
                                 "2" = "foraging",
                                 "3" = "exploratory")
  ) %>%
  group_by(viterbi, hour(date)) 

# use this
ggplot(budget, aes(hour(date))) +
  facet_grid(viterbi ~ ag.class.mean) + 
...
ele.df <- filter(output, ID == "Lowana") %>%
  mutate(state = as.numeric(viterbi), row.id = seq.int(nrow(.))) %>%
  drop_na(x,y) %>%
  droplevels()
...
mapview(ele.sf, zcol = "state", col.region = pal, cex = 0.1, alpha = 0, legend = TRUE) + # for legend, make points super small
  mapview(GSME, col.regions="darkgreen", color="darkgreen", alpha.regions=0.25) +
  mapview(ele.sldf, color = "grey", cex = 0.5) +
  mapview(filter(ele.sf, state == 3), color = pal[3], cex = 0.5, alpha = 1, legend = FALSE) +
  mapview(filter(ele.sf, state == 2), color = pal[2], cex = 0.5, alpha = 1, legend = FALSE) +
  mapview(filter(ele.sf, state == 1), color = pal[1], cex = 0.5, alpha = 1, legend = FALSE) 
...
{df <- output %>% mutate(state = recode_factor(viterbi,
  "1" = "encamped", "2" = "exploratory", "3" = "goal-oriented"))
state.split <- split(df, df$viterbi)
lapply(state.split, plotRA)}

# for saving 
{tiff("rosePlot_1.tiff", res = 200)
...
tiff("step_angle_plot.tiff", res = 300, width = 8, height = 5, units = "in")
ggplot(df, aes(x = dist, y = rel.angle)) +
  geom_point(aes(color = state)) + scale_color_manual(values = pal)
dev.off()
