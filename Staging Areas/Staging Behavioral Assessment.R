...
cor.test(window$stage.pct, window$window.length)
...
mcp <- lapply(split, adehabitatHR::mcp)
...
gme.stage$year.mcp.area <- mcp.areas@data$year.mcp.area
...
## Summarise all data
mod.df <- gme.stage %>%
  group_by(subject_name, year.cuts, tactic.season, subject_sex, subject_ageClass, mean.bout, year.mcp.area) %>%
  summarise(
    mean.ag = mean(ag.used),
    mu.dailyDist = median(dailyDist),
...
mod.2 <- lmer(pct.stage ~ log(mu.dailyDist) + log(year.mcp.area) + (1|subject_name), data = mod.df, REML = FALSE)
summary(mod.2)
plot(mod.2)
...
mod.4 <- lmer(pct.stage ~ mean.ag + mean.bout + log(year.mcp.area) + log(mu.dailyDist) + (1|subject_name), data = mod.df, REML = FALSE)
summary(mod.4)
...
mod.5 <- lmer(pct.stage ~ mean.ag + (1|subject_name), data = mod.df)
mod.6 <- lmer(pct.stage ~ mean.ag + log(mu.dailyDist) + (1|subject_name), data = mod.df, REML = FALSE)
mod.7 <- lmer(pct.stage ~ mean.ag + log(year.mcp.area) + (1|subject_name), data = mod.df, REML = FALSE)
mod.8 <- lmer(pct.stage ~ mean.ag + log(year.mcp.area) + log(mu.dailyDist) + (1|subject_name), data = mod.df, REML = FALSE)
mod.9 <- lmer(pct.stage ~ mean.ag + mean.bout + log(mu.dailyDist) + (1|subject_name), data = mod.df, REML = FALSE)
mod.10 <- lmer(pct.stage ~ log(mu.dailyDist) + (1|subject_name), data = mod.df, REML = FALSE)

library(MuMIn)
# AICc
t <- as.data.frame(AICc(mod.global, mod.1, mod.2, mod.3, mod.4, mod.5, mod.6, mod.7, mod.8, mod.9, mod.10))
...
t$loglik <- c(logLik(mod.global), logLik(mod.1), logLik(mod.2), logLik(mod.3), logLik(mod.4), logLik(mod.5), logLik(mod.6), logLik(mod.7), logLik(mod.8), logLik(mod.9), logLik(mod.10))
t$mod.formula <- c(mod.global@call$formula, mod.1@call$formula, mod.2@call$formula, mod.3@call$formula, mod.4@call$formula, 
  mod.5@call$formula, mod.6@call$formula, mod.7@call$formula, mod.8@call$formula, mod.9@call$formula, mod.10@call$formula)
t <- t[order(t$AICc),]
t
...
sjPlot::tab_model(mod.6)
sjPlot::plot_model(mod.6)
