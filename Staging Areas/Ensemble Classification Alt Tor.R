...
result.matrix <- readRDS("~/Dropbox/CSU/GME_Movement/Staging Areas/tor loop tests/AltTorloop_w2_6to21_seq7to2_agmask01_20220613.RDS")

# convert matrix to dataframe for viz
result.df <- as.data.frame(result.matrix)
result.df <- mutate_at(result.df, c('ag.window.ext', 'n.stage', 'n.stage.adj', 'tor.seq', 'n.stage.ratio',
...
result.df <- result.df %>%
  ...
  filter(tor.seq == min(tor.seq))

#### Plot Loop Results ####

plot.df <- filter(result.df, ag.window.ext == 1)

ggplot(plot.df, aes(x = tor.seq, y = (1-n.stage.err), group = as.factor(win.start))) +
  geom_line(aes(color = as.factor(win.start))) +
  facet_wrap(.~win.siz) +
  xlab('Mean NSD in window') + ylab('accuracy') + labs(color = 'window start time') +
  ggtitle('Tor Algorithm accuracy stratified by window size (hours)')
...
gme <- readRDS('./Staging Areas/tor loop tests/GMEcollars_004_stageclassified_alt.tor.3.filter_20220613.RDS')
...
for (i in 1:length(split)){
  ...
  for (j in 1:dim(split[[i]])[1]) { # voting by row of split df -- row corresponds with the parameter set and accuracy for each model
    ...
    df2 <- gme %>%
      ...
      # mean tor
      mutate(mean.tor = mean(log(tor))) 
    
    # tag all staging events
    stage.event <- ifelse(df2$mean.tor <= tor.threshold & df2$stage.period == 1, 1, 0)
    # tag ag staging events
    ag.stage.event <- ifelse(df2$mean.tor <= tor.threshold & df2$stage.period == 1 & df2$ag.window.ext == 1, 1, 0)
...
saveRDS(sums, 'Staging Areas/tor loop tests/alttor_ensemble_sums.all_agmask_v2_20220614.RDS')
saveRDS(sums.all, 'Staging Areas/tor loop tests/alttor_ensemble_sums.all_agmask_v2_20220614.RDS')
...
sums.all <- readRDS('Staging Areas/tor loop tests/alttor_ensemble_sums.all_agmask_v2_20220614.RDS')
...
gme$vote.ag <- ifelse(gme$ag.window.ext == 1, gme$vote, 0)
gme$vote.nonag <- ifelse(gme$ag.window.ext == 0, gme$vote, 0)

## index ag staging events
eventFlag <- ifelse(gme$vote.ag == 1, TRUE, FALSE)
eventIndex <- inverse.rle(within.list(rle(eventFlag),
                                      values[values] <- seq_along(values[values])))
# assign a unique event index for each stage event
gme$stage.event.index <- eventIndex
...
length(unique(gme$stage.event.index))
...
stage.relocs <- filter(gme, vote.ag == 1)
...
write.csv(gme,'./Staging Areas/movdata/GMEcollars_004_stageclassified_agmask_ALTTOR_20220103.csv')
