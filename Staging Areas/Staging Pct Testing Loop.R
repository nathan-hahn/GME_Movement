...
# TODO: days that used ag after 3pm - exclude days with ag use leaking over
...
t.pct$ag.window <- as.factor(t.pct$ag.window.ext)
levels(t.pct$ag.window) <- c('non-window', 'ag-window')
...
tag_stage <- function(df, pct.seq){
  ...
  df$stage.event <- ifelse(df$pct > pct.seq & df$stage.period == 1 & df$dw.day == 0, 1, 0)
  # tag ag staging events
  df$ag.stage.event <- ifelse(df$pct > pct.seq & df$stage.period == 1 & df$dw.day == 0 & df$ag.window.ext == 1, 1, 0)
...
pct.seq = seq(0.01, 1, 0.01) # sequence of possible E-M ratios (used in k loop)
...
for(i in 1:11){ # window size loop - index adds to hr.start 
  ...
  for (j in 1:dim(hr.win)[1]) { # window period loop indexes the matrix row to define stage period
    ...
    for (k in 1:length(pct.seq)){
      ...
      stage.table <- tag_stage(gme.stage, pct.seq[[k]])
      
      # add reference for window size and pct
